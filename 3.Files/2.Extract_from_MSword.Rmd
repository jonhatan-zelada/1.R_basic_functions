...
4.Cells positions and values are dispatched in columns row_id, cell_id, text and is_header
...
5.Reshaping the data close to the orginal table
...
6.Getting headers requires another operation:
```{r}
...
View the field which contain:
```{r}
View(docxtest$text[docxtest$row_id==27])
```

Is "X" before or after some expression like "Evento de Uso 1"  
"Evento de Uso 1 (Situación no esperada para escalar a REAPRO)X"
```{r}
pattern="Evento de Uso 1 (Situación no esperada para escalar a REAPRO)X"
